...
/*** * fn_asideFilterEvent (25.03.27) * ***/
const fn_asideFilterEvent = {
  areaToggle() {
    const wrapToggleBtns = document.querySelectorAll(".aside-filter-area .btn-toggle-filter");
    if (!wrapToggleBtns.length) return;

    wrapToggleBtns.forEach(btn => {
      btn.addEventListener("click", function() {
        const filterWrap = this.closest('.in-page-filter-wrap');
        if (filterWrap.classList.contains("fold")) {
            filterWrap.classList.remove("fold");
            this.querySelector(".sr-only").textContent = "닫기";
        } else {
            filterWrap.classList.add("fold");
            this.querySelector(".sr-only").textContent = "열기";
        }
      })
    })
  },
  ...
  listToggle() {
    const listToggleBtns = document.querySelectorAll(".btn-toggle");
    if (!listToggleBtns.length) return;
  ...
  scrollFixed() {
    const filterWraps = document.querySelectorAll('.in-page-filter-wrap');
    if (!filterWraps.length) return;

    filterWraps.forEach(wrap => {
      if (filterWraps.length == 1 ) {
        let fixPosition = wrap.getBoundingClientRect().top + window.pageYOffset;

        if (window.pageYOffset >= fixPosition) wrap.classList.add('fix');
        else wrap.classList.remove('fix');
      }

      else {
        const activeFilter = document.querySelector(".tab-conts.active").querySelector(".in-page-filter-wrap");
        let fixPosition = activeFilter.getBoundingClientRect().top + window.pageYOffset;

        if (window.pageYOffset >= fixPosition) {
          for (let i = 0; i < filterWraps.length; i++) {
            filterWraps[i].classList.add('fix');
          }
        } else {
          for (let i = 0; i < filterWraps.length; i++) {
            filterWraps[i].classList.remove('fix');
          }
        }
      }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fn_asideFilterEvent.areaToggle();
  fn_asideFilterEvent.listToggle();
...
window.addEventListener("scroll", () => {
  ...
  krds_helpPanel.init();
  fn_asideFilterEvent.scrollFixed();
});
