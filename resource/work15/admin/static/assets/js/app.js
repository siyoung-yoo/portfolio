...
const App = function () {
    ...
    const sidebarMainToggle = function() {
        // Elements
        const sidebarToggler = $('.topnav-hamburger');
        const htmlElement = document.querySelector('html');
        const verticalOverlay = document.querySelector('.vertical-overlay');

        sidebarToggler.click(function() {
            const layoutMode = htmlElement.getAttribute('data-layout');
            const sidebarSize = htmlElement.getAttribute('data-sidebar-size');
            const newSidebarSize = sidebarSize === 'lg' ? 'sm' : 'lg';
            const hamburgerIcon = $('.hamburger-icon');
            const sideNav = $('.navbar-menu');

            if (window.innerWidth > 768 && layoutMode === 'vertical') {
                htmlElement.setAttribute('data-sidebar-size', newSidebarSize);
                hamburgerIcon.toggleClass('open');
                fitDatatableHead();
            } else if (window.innerWidth <= 768 && layoutMode === 'vertical') {
                document.querySelector('body').classList.add('vertical-sidebar-enable');
                verticalOverlay.style.display = "block"
            }

            if (window.innerWidth > 1024 && layoutMode === 'horizontal') {

            } else if (window.innerWidth <= 1024 && layoutMode === 'horizontal') {
                document.querySelector('body').classList.toggle('horizontal-menu-enable');
            }
        });
    };
    ...
    const dataSizeMode = () => {
        ...
        if (layoutMode === 'vertical') {
            if (window.innerWidth <= 768) {
                ...
                if (sidebarSize === 'lg' && hamburgerIcon) {
                    hamburgerIcon.classList.remove('open');
                    if (window.innerWidth > 768 && verticalOverlay) {
                        verticalOverlay.style.display = "none";
                        document.body.classList.remove('vertical-sidebar-enable');
                    }
                } else if (sidebarSize === 'sm' && hamburgerIcon) {
                    hamburgerIcon.classList.add('open');
                }
            }
        }

        if(layoutMode === 'horizontal' && window.innerWidth > 1024) {
            document.querySelector('body').classList.remove('horizontal-menu-enable');
        }
    }
    window.addEventListener('resize', dataSizeMode);
    if (document.querySelector('html').getAttribute('data-layout') === 'vertical') {
        document.querySelector('.vertical-overlay').addEventListener('click', function() {
            document.body.classList.remove('vertical-sidebar-enable');
            document.querySelector('.vertical-overlay').style.display = 'none';
        });
    }
    ...
    const navigationSidebar = function() {
        ...
        // 가로형 메뉴에서 네비게이션 밖의 영역 클릭 시 네비게이션 항목 닫기
        $(document).on('click', function(event) {
            if ($('html').attr('data-layout') === 'horizontal') {
                if (!$(event.target).closest('.' + navClass + ' .nav-item-1st').length) {
                    $('.' + navClass + ' .' + navItemOpenClass)
                        .removeClass(navItemOpenClass)
                        .children('.' + navSubmenuClass).slideUp(navSlidingSpeed);
                }
            }
        });
    };
...
//		    var navbarHeight = $('.navbar-static').height();
//		    var navbarHeightSub = $('.navbar-static-sub').height();

//		    var searchOptionTitleHeight = $('.page-content .content .card-header.header-elements-inline').height();
//		    var searchOptionHeight = $('.page-content .content .box-input').height();

//            if( $('#moreInfoArea').hasClass('more-info-area') ){
//                var moreInfoHeight = $('.more-info-area').outerHeight();
//            }else{
//                var moreInfoHeight = 0;
//            }

//			var dataTableBodyHeight = windowHeight - navbarHeight - navbarHeightSub - searchOptionTitleHeight - searchOptionHeight - moreInfoHeight;
            ...
            if (parentContainer.hasClass(cardCollapsedClass)) {
				...
				if (windowWidth <= 991) {
//				    $('.datatable-scroll').css('height','100%');
//				    $('.dataTables_scrollBody').css('height','100%');
				}else{
				    if( $('.card').hasClass('datatable-destroy') ){
				        //$(this).destroyDatatable();
				    }else{
//					    $('.datatable-scroll').css({height : dataTableBodyHeight - 107 + 'px'});
//		                $('.dataTables_scrollBody').css({height : dataTableBodyHeight - 143 + 'px'});
	                }
				}
            }
            else {
...
//					    $('.datatable-scroll').css({height : dataTableBodyHeight + searchOptionHeight - 97 + 'px'});
//		                $('.dataTables_scrollBody').css({height : dataTableBodyHeight + searchOptionHeight - 133 + 'px'});
	                }
				}
            }
        });
